<template>
  <view v-if="isShow" class="lucky-box" :style="{ width: boxWidth + 'px', height: boxHeight + 'px' }">
    ...
    <!-- 我新增的动画效果 -->
    <view :class="['my-lucky-btn',showMyLuckyBtnAnim===true&&'active']"></view>
...
<style scoped>
  ...
  .my-lucky-btn {
    ...
    width: 234rpx;
    height: 234rpx;
    background-color: rgba(234, 204, 255, .8);
    border-radius: 50%;
    filter: blur(5px); /* 边缘虚化效果*/
    opacity: 0;
    &.active {
      animation: btnScaleFade 1s linear 1 forwards;
    }
  }
  @keyframes btnScaleFade {
    0% {
      transform: scale(1);
      opacity: 1;
      filter: blur(5px);
