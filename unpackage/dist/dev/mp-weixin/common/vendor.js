...
const hasOwn$3 = (val, key) => hasOwnProperty$2.call(val, key);
...
const replacer = (_key, val) => {
  ...
  } else if (isObject$1(val) && !isArray$4(val) && !isPlainObject(val)) {
    return String(val);
  }
  return val;
};
...
function validateProp$1(name, value, prop, props, isAbsent) {
  ...
  if (type2 != null && type2 !== true && !skipCheck) {
    let isValid = false;
    const types = isArray$4(type2) ? type2 : [type2];
    const expectedTypes = [];
...
function patchMPEvent(event, instance) {
  if (event.type && event.target) {
    ...
    if (isPlainObject(event.detail) && hasOwn$3(event.detail, "checked") && !hasOwn$3(event.detail, "value")) {
...
function initRuntimeSocketService() {
  const hosts = "127.0.0.1,192.168.10.103";
  const port = "8090";
  const id = "mp-weixin_VnWhV3";
...
function parsePropType(type2, defaultValue) {
  if (isArray$4(type2) && type2.length === 1) {
    return type2[0];
  }
  return type2;
}
